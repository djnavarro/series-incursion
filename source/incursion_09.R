...
create_base_image <- function(seed) {
  ...
  values <- tibble::tibble(
    x = base$x,
    y = base$y,
    xend = x + curl$x * max_curl,
    yend = y + curl$y * max_curl,
    ...
    width = width_scale * runif(n_ribbons, min = .01, max = .5),
    smooth = 6L,
    n = 100L,
  ...
  ht$val <- ambient::normalise(ht$val)
...
if (TRUE) for (s in 1800:1899) make_art(s)
