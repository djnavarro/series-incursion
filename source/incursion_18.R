...
unfold <- function(
  ...
  do_step <- function(data, iter) {
    ...
    data <- dplyr::filter(data, x^2 + y^2 < 2 | free)
...
make_art <- function(seed) {
  ...
  pic <- ggplot2::ggplot(dat) +
    ...
    ggplot2::coord_cartesian(
      xlim = compute_limit(dat, "x", border = .75),
      ylim = compute_limit(dat, "y", border = .75)
    ) +
...
if (TRUE) for (s in 2700:2799) make_art(s)
