...
create_base_image <- function(seed) {
  ...
  is_vertical <- runif(1) < .5
  max_curl <- 2

  values <- tibble::tibble(
    x = base$x,
    y = base$y,
    xend = x + !is_vertical * curl$x * max_curl,
    yend = y + is_vertical * curl$y * max_curl,
    ...
    width = width_scale * runif(n_ribbons, min = .01, max = .3),
    smooth = 6L,
    n = 100L,
...
if (TRUE) for (s in 1813:1899) make_art(s)
